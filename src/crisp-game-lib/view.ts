...
let gameDescriptionCanvas: HTMLCanvasElement;
let gameDescriptionContext: CanvasRenderingContext2D;
let mainContent: CanvasRenderingContext2D;
const gameDescriptionCanvasSize = new Vector(100, 6 * 5);
...
`;
  const gameDescriptionCanvasCss = `
position: absolute;
left: 50%;
top: 12%;
transform: translate(-50%, -50%);
`;
  ...
  if (theme.isUsingPixi) {
    ...
    mainContent = context = canvas.getContext("2d");
    context.imageSmoothingEnabled = false;
    canvas.style.cssText = canvasCss + crispCss;
  }
  document.body.appendChild(canvas);
  gameDescriptionCanvas = document.createElement("canvas");
  gameDescriptionContext = gameDescriptionCanvas.getContext("2d");
  gameDescriptionCanvas.width = gameDescriptionCanvasSize.x;
  gameDescriptionCanvas.height = gameDescriptionCanvasSize.y;
  gameDescriptionCanvas.style.cssText = gameDescriptionCanvasCss + crispCss;
  document.body.appendChild(gameDescriptionCanvas);
...
export function drawToMain() {
  context = mainContent;
}

export function drawToDescription() {
...
export function terminate() {
  ...
  document.body.removeChild(gameDescriptionCanvas);
  canvas = undefined;
}
...
function setSize() {
  ...
  gameDescriptionCanvas.style.width = `${Math.floor(
    (cw * gameDescriptionCanvasSize.x) / size.x
  )}px`;
  gameDescriptionCanvas.style.height = `${Math.floor(
    (ch * gameDescriptionCanvasSize.y) / size.y
  )}px`;
}
